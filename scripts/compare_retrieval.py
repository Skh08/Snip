...
def main() -> None:
    ...
    vector_correct = 0
    hybrid_correct = 0
    grounded_cases = [case for case in cases if case.get("expected_grounded") is not False]
    ...
    print(f"Vector-only: {vector_correct}/{len(grounded_cases)}")
    print(f"Hybrid: {hybrid_correct}/{len(grounded_cases)}")
